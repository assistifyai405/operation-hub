...
import { formatCurrency, localeTag } from "@/i18n/format";
...
function InvoiceView({ inv, testidPrefix = "invoice-view" }) {
  ...
  const money = (v) => formatCurrency(v || 0, locale, "EUR");
  const c = inv.content || {};
  ...
  return (
    <div className="space-y-4" data-testid={testidPrefix}>
      <div className="grid grid-cols-2 gap-4 rounded-xl border border-white/10 bg-zinc-950 p-5 text-sm">
        <div><p className="text-xs uppercase tracking-wide text-zinc-500">{t("writers.invoice.billTo")}</p><p className="mt-1 font-medium text-zinc-100">{c.company || c.client_name || "—"}</p><p className="whitespace-pre-wrap text-zinc-400">{c.billing_address}</p></div>
        <div className="text-right"><p className="text-xs uppercase tracking-wide text-zinc-500">{t("writers.invoice.details")}</p><p className="mt-1 text-zinc-300">{t("writers.invoice.issue")}: {c.issue_date || "—"}</p><p className="text-zinc-300">{t("writers.invoice.due")}: {c.due_date || "—"}</p></div>
      </div>
      <div className="overflow-hidden rounded-xl border border-white/10 bg-zinc-950">
        <table className="w-full text-sm">
          <thead className="bg-brand-600/15 text-zinc-300"><tr><th className="px-4 py-2 text-left font-medium">{t("writers.invoice.description")}</th><th className="px-4 py-2 text-right font-medium">{t("writers.invoice.quantity")}</th><th className="px-4 py-2 text-right font-medium">{t("writers.invoice.unitPrice")}</th><th className="px-4 py-2 text-right font-medium">{t("writers.invoice.amount")}</th></tr></thead>
          <tbody>
      ...
      <div className="ml-auto w-full max-w-xs space-y-1 text-sm">
        <div className="flex justify-between text-zinc-400"><span>{t("writers.invoice.subtotal")}</span><span>{money(totals.subtotal)}</span></div>
        <div className="flex justify-between text-zinc-400"><span>{t("writers.invoice.vat")} ({c.vat_rate || 0}%)</span><span>{money(totals.vat)}</span></div>
        <div className="flex justify-between border-t border-brand-500/40 pt-1 text-base font-semibold text-zinc-50"><span>{t("writers.invoice.total")}</span><span data-testid={`${testidPrefix}-total`}>{money(totals.total)}</span></div>
      </div>
      {["payment_terms", "bank_details", "notes"].filter((k) => c[k]).map((k) => (
        <div key={k} className="rounded-xl border border-white/10 bg-zinc-950 p-4"><p className="text-xs uppercase tracking-wide text-brand-400">{t(`writers.invoice.fields.${k}`)}</p><p className="mt-1 whitespace-pre-wrap text-sm text-zinc-300">{c[k]}</p></div>
      ))}
    </div>
  );
}
...
export default function InvoiceWriter({ projectId, projectName, onSaved }) {
  ...
  const sendToClient = async () => {
    ...
    try {
      const doc = await invoiceWriterApi.save(projectId, { invoice_number: number, title: title || t("writers.invoice.defaultTitle", { project: projectName }), status: "Sent", content, line_items: lineItems });
      setInvoice(doc); setDirtyVersion(doc.version);
      toast.success(t("writers.invoice.markedSent")); onSaved?.();
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const doc = await invoiceWriterApi.save(projectId, { invoice_number: number, title: title || t("writers.invoice.defaultTitle", { project: projectName }), status, content, line_items: lineItems });
      setInvoice(doc); setNumber(doc.invoice_number); setLineItems(doc.line_items); setDirtyVersion(doc.version); setEditing(false);
      toast.success(t("writers.savedVersion", { type: t("writers.invoice.name"), version: doc.version })); onSaved?.();
  ...
  const exportFile = (fmt) => { window.open(invoiceWriterApi.exportUrl(projectId, fmt), "_blank"); toast.success(t("writers.exporting", { format: fmt.toUpperCase() })); setTimeout(() => onSaved?.(), 1500); };
  ...
  useAssistantDocument({ type: "invoice", name: title || t("writers.invoice.defaultTitle", { project: projectName }), sections: BODY_FIELDS, content, setField, active: !!content });
    ...
    if (generating) {
      return (
        <div data-testid="invoice-generating">
          <AIWorkflow running={generating} title={t("writers.invoice.workflow.title")} steps={t("writers.invoice.workflow.steps", { returnObjects: true })} />
        </div>
      );
    }
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-white/10 bg-zinc-950/40 py-20 text-center" data-testid="invoice-empty">
        ...
        <h3 className="mt-5 text-lg font-semibold text-zinc-100">{t("writers.invoice.emptyTitle")}</h3>
        <p className="mt-1 max-w-md text-sm text-zinc-500">{t("writers.invoice.emptyDescription")}</p>
        <button onClick={generate} disabled={generating} data-testid="generate-invoice-btn" className="mt-6 flex items-center gap-2 rounded-lg bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white transition-all hover:bg-brand-500 disabled:opacity-60 glow-brand">
          {generating ? <><Loader2 className="h-4 w-4 animate-spin" /> {t("writers.invoice.generating")}</> : <><Sparkles className="h-4 w-4" /> {t("writers.invoice.generate")}</>}
        </button>
      </div>
    );
  }
    ...
    <div className="space-y-5" data-testid="tab-invoice">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-zinc-500">{t("writers.version")} <b className="text-zinc-300" data-testid="invoice-version">{dirtyVersion === null ? t("writers.draft") : dirtyVersion}</b></span>
            </div>
            <input value={title} onChange={(e) => setTitle(e.target.value)} data-testid="invoice-title-input" placeholder={t("writers.invoice.titlePlaceholder")}
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex flex-col"><label className="text-[10px] uppercase text-zinc-500">{t("writers.invoice.issue")}</label>
            ...
            <div className="flex flex-col"><label className="text-[10px] uppercase text-zinc-500">{t("writers.invoice.due")}</label>
            ...
            <div className="flex flex-col"><label className="text-[10px] uppercase text-zinc-500">{t("common.status")}</label>
              <Select value={status} onValueChange={setStatus}>
                ...
                <SelectContent className="border-white/10 bg-zinc-900 text-zinc-100">{statuses.map((s) => <SelectItem key={s} value={s} data-testid={`invoice-status-${s}`}>{t(`statuses.${s}`, { defaultValue: s })}</SelectItem>)}</SelectContent>
              </Select></div>
          </div>
        </div>
      </div>
      ...
      {showHistory && (
        <div className="rounded-xl border border-white/10 bg-zinc-950 p-4" data-testid="invoice-version-list">
          ...
          {history.length === 0 ? <p className="text-xs text-zinc-500" data-testid="invoice-no-versions">{t("writers.noVersions")}</p> : (
                ...
                <div key={v.version} className={`rounded-lg border p-3 ${dirtyVersion === v.version && !compareWith ? "border-brand-500/50 bg-brand-600/10" : "border-white/10 bg-zinc-900"}`} data-testid={`invoice-version-${v.version}`}>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => setCompareWith(v)} data-testid={`compare-invoice-version-${v.version}`} title={t("writers.compareCurrent")} className="rounded p-1 text-zinc-500 hover:text-brand-400"><GitCompare className="h-3.5 w-3.5" /></button>
                      <button onClick={() => restore(v.version)} data-testid={`restore-invoice-version-${v.version}`} title={t("writers.restore")} className="rounded p-1 text-zinc-500 hover:text-emerald-400"><RotateCcw className="h-3.5 w-3.5" /></button>
                    </div>
                  </div>
      ...
      {compareWith ? (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2" data-testid="invoice-compare">
          <div><div className="mb-3 flex items-center justify-between rounded-lg bg-zinc-900 px-3 py-2"><span className="text-sm font-semibold text-zinc-300">v{compareWith.version} · {money(compareWith.total)}</span><button onClick={() => setCompareWith(null)} data-testid="exit-compare-invoice-btn" className="text-xs text-zinc-500 hover:text-white">{t("writers.exitCompare")}</button></div><InvoiceView inv={compareWith} testidPrefix="invoice-compare-a" /></div>
          <div><div className="mb-3 rounded-lg bg-brand-600/20 px-3 py-2 text-sm font-semibold text-brand-200">{dirtyVersion === null ? t("writers.currentDraft") : `v${dirtyVersion}`}</div><InvoiceView inv={{ content, line_items: lineItems }} testidPrefix="invoice-compare-b" /></div>
        </div>
      ) : editing ? (
        <div className="space-y-4">
          ...
          <div className="overflow-hidden rounded-xl border border-white/10 bg-zinc-950" data-testid="invoice-line-items">
            <table className="w-full text-sm">
              <thead className="bg-zinc-900 text-zinc-400"><tr><th className="px-3 py-2 text-left font-medium">{t("writers.invoice.description")}</th><th className="px-3 py-2 text-right font-medium">{t("writers.invoice.quantity")}</th><th className="px-3 py-2 text-right font-medium">{t("writers.invoice.unitPrice")}</th><th className="px-3 py-2 text-right font-medium">{t("writers.invoice.amount")}</th><th /></tr></thead>
              <tbody>
                {lineItems.map((li, i) => (
            ...
            <button onClick={addItem} data-testid="invoice-add-item-btn" className="flex w-full items-center justify-center gap-1.5 border-t border-white/5 py-2 text-sm font-medium text-brand-400 hover:bg-zinc-900"><Plus className="h-4 w-4" /> {t("writers.invoice.addLineItem")}</button>
          </div>
          {/* Totals + VAT */}
          <div className="ml-auto w-full max-w-xs space-y-2 rounded-xl border border-white/10 bg-zinc-950 p-4 text-sm">
            <div className="flex items-center justify-between text-zinc-400"><span>{t("writers.invoice.subtotal")}</span><span data-testid="invoice-subtotal">{money(totals.subtotal)}</span></div>
            <div className="flex items-center justify-between text-zinc-400"><span>{t("writers.invoice.vat")} %</span><input type="number" value={content.vat_rate || 0} onChange={(e) => setField("vat_rate", Number(e.target.value))} data-testid="invoice-vat-input" className="w-20 rounded border border-white/10 bg-zinc-900 px-2 py-1 text-right text-zinc-200 outline-none focus:border-brand-500" /></div>
            <div className="flex justify-between text-zinc-400"><span>{t("writers.invoice.vatAmount")}</span><span data-testid="invoice-vat-amount">{money(totals.vat)}</span></div>
            <div className="flex justify-between border-t border-brand-500/40 pt-2 text-base font-semibold text-zinc-50"><span>{t("writers.invoice.total")}</span><span data-testid="invoice-total">{money(totals.total)}</span></div>
          </div>
          {/* Editable text fields */}
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {BODY_FIELDS.map((f) => (
              <div key={f.key} className="rounded-xl border border-white/10 bg-zinc-950 p-4">
                <label className="mb-1.5 block text-xs uppercase tracking-wide text-brand-400">{t(`writers.invoice.fields.${f.key}`)}</label>
                {f.area
