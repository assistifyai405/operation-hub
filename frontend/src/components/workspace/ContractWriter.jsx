  ...
  if (!content) {
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-white/10 bg-zinc-950/40 py-20 text-center" data-testid="contract-empty">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-violet-600 glow-violet animate-pulse-glow"><ScrollText className="h-8 w-8 text-white" /></div>
        ...
        <button onClick={generate} disabled={generating} data-testid="generate-contract-btn" className="mt-6 flex items-center gap-2 rounded-lg bg-violet-600 px-5 py-2.5 text-sm font-semibold text-white transition-all hover:bg-violet-500 disabled:opacity-60 glow-violet">
  ...
  return (
    <div className="space-y-5" data-testid="tab-contract">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button onClick={() => setEditing((e) => !e)} data-testid="edit-contract-btn" className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-sm font-medium transition-all ${editing ? "border-violet-500 bg-violet-600/15 text-violet-300" : "border-white/10 bg-zinc-900 text-zinc-300 hover:text-white"}`}>
        ...
        <button onClick={() => setShowHistory((s) => !s)} data-testid="contract-version-history-btn" className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-sm font-medium transition-all ${showHistory ? "border-violet-500 bg-violet-600/15 text-violet-300" : "border-white/10 bg-zinc-900 text-zinc-300 hover:text-white"}`}>
        ...
        <button onClick={save} disabled={saving} data-testid="save-contract-btn" className="flex items-center gap-1.5 rounded-lg bg-violet-600 px-3 py-1.5 text-sm font-semibold text-white transition-all hover:bg-violet-500 disabled:opacity-60 glow-violet">
          ...
          {history.length === 0 ? <p className="text-xs text-zinc-500" data-testid="contract-no-versions">No versions yet. Save to create v1.</p> : (
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {[...history].reverse().map((v) => (
                <div key={v.version} className={`rounded-lg border p-3 ${dirtyVersion === v.version && !compareWith ? "border-violet-500/50 bg-violet-600/10" : "border-white/10 bg-zinc-900"}`} data-testid={`contract-version-${v.version}`}>
                  <div className="flex items-center justify-between">
                    <button onClick={() => viewVersion(v)} data-testid={`view-contract-version-${v.version}`} className="text-sm font-medium text-zinc-200 hover:text-violet-300">v{v.version}</button>
                    <div className="flex items-center gap-1">
                      <button onClick={() => setCompareWith(v)} data-testid={`compare-contract-version-${v.version}`} title="Compare with current" className="rounded p-1 text-zinc-500 hover:text-violet-400"><GitCompare className="h-3.5 w-3.5" /></button>
