...
export default function ProposalWriter({ projectId, projectName, onSaved }) {
  ...
  const sendToClient = async () => {
    ...
    try {
      const doc = await proposalWriterApi.save(projectId, { title: title || `${projectName} — Proposal`, status: "Sent", content });
                ...
                <div key={v.version} className={`rounded-lg border p-3 ${dirtyVersion === v.version && !compareWith ? "border-brand-500/50 bg-brand-600/10" : "border-white/10 bg-zinc-900"}`} data-testid={`proposal-version-${v.version}`}>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => setCompareWith(v)} data-testid={`compare-proposal-version-${v.version}`} title="Compare with current" className="rounded p-1 text-zinc-500 hover:text-brand-400"><GitCompare className="h-3.5 w-3.5" /></button>
