    ...
    <div className="assistant-md text-sm leading-relaxed text-zinc-200">
      <ReactMarkdown
        ...
        components={{
          ...
          li: ({ children, ...rest }) => (
            <li className="flex gap-2" {...rest}><span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-brand-400" /><span className="min-w-0">{children}</span></li>
          ),
          strong: (p) => <strong className="font-semibold text-zinc-50" {...p} />,
          a: (p) => <a className="text-brand-300 underline underline-offset-2 hover:text-brand-200" target="_blank" rel="noreferrer" {...p} />,
          code: ({ inline, children, ...rest }) =>
            inline
              ? <code className="rounded bg-zinc-800 px-1 py-0.5 text-[12px] text-brand-200" {...rest}>{children}</code>
          ...
          blockquote: (p) => <blockquote className="border-l-2 border-brand-500/50 pl-3 text-zinc-400" {...p} />,
