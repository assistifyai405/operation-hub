...
function Report({ report }) {
  ...
  return (
    <div className="mt-2.5 rounded-lg border border-brand-500/20 bg-brand-500/[0.05] p-3" data-testid="assistant-report">
      {hasDetail && (
        <div className="space-y-1.5">
          {report.what && <p className="text-xs text-zinc-300"><span className="font-semibold text-brand-300">What: </span>{report.what}</p>}
          {report.why && <p className="text-xs text-zinc-300"><span className="font-semibold text-brand-300">Why: </span>{report.why}</p>}
          {report.impact && <p className="text-xs text-zinc-300"><span className="font-semibold text-brand-300">Impact: </span>{report.impact}</p>}
        </div>
      )}
      ...
      <AnimatePresence>
        {!open && (
          <motion.button
            ...
            className="fixed bottom-24 right-6 z-[60] flex h-14 w-14 items-center justify-center rounded-2xl bg-brand-600 text-white shadow-lg shadow-brand-900/40 glow-brand"
            aria-label="Open Assistify Assistant">
            <Sparkles className="h-6 w-6" />
            <span className="absolute inset-0 rounded-2xl bg-brand-500/40 animate-ping-slow" />
          </motion.button>
        )}
      </AnimatePresence>
        ...
        {open && (
          <motion.div
            ...
            <div className="flex items-center gap-3 border-b border-white/10 px-4 py-3.5">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-brand-600 glow-brand"><Sparkles className="h-5 w-5 text-white" /></span>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-bold text-zinc-50">Assistify Assistant</p>
                <p className="truncate text-xs text-zinc-500" data-testid="assistant-context-line">I see you're on <span className="text-brand-300">{ctxLabel}</span></p>
              </div>
              ...
              <div className="flex items-center gap-2 border-b border-white/10 px-4 py-2">
                ...
                <div className="relative flex-1">
                  <select value={targetKey} onChange={(e) => setTargetKey(e.target.value)} data-testid="assistant-section-select"
                    className="w-full appearance-none rounded-lg border border-white/10 bg-zinc-900 py-1.5 pl-2.5 pr-7 text-xs text-zinc-200 focus:border-brand-500/50 focus:outline-none">
            ...
            <div ref={scrollRef} className="flex-1 space-y-4 overflow-y-auto px-4 py-4" data-testid="assistant-messages">
              {messages.length === 0 && (
                <div className="rounded-xl border border-white/10 bg-zinc-900/40 p-4 text-center" data-testid="assistant-welcome">
                  ...
                  <p className="mx-auto mt-1 max-w-[16rem] text-xs text-zinc-500">Pick a suggestion below, type <span className="text-brand-300">/</span> for commands, or ask me anything about {ctxLabel}.</p>
                </div>
              )}
              {messages.map((m) => (
                <div key={m.id} className={m.role === "user" ? "flex justify-end" : ""} data-testid={`assistant-msg-${m.role}`}>
                  {m.role === "user" ? (
                    <div className="max-w-[85%] rounded-2xl rounded-br-sm bg-brand-600 px-3.5 py-2 text-sm text-white">{m.content}</div>
                  ) : (
                    <div className="max-w-[92%]">
                      {m.error ? (
                        ...
                        <div className="rounded-2xl rounded-bl-sm border border-white/10 bg-zinc-900/70 px-3.5 py-3">
                          {m.content ? <Markdown>{m.content}</Markdown> : <span className="inline-flex gap-1"><span className="h-1.5 w-1.5 animate-bounce rounded-full bg-brand-400" /><span className="h-1.5 w-1.5 animate-bounce rounded-full bg-brand-400" style={{ animationDelay: "0.15s" }} /><span className="h-1.5 w-1.5 animate-bounce rounded-full bg-brand-400" style={{ animationDelay: "0.3s" }} /></span>}
                          {!m.typing && <Report report={m.report} />}
                          {!m.typing && (m.content || m.apply) && (
                            <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
                              {m.apply && !m.applied && (
                                <button onClick={() => applyMsg(m)} data-testid="assistant-apply-btn" className="inline-flex items-center gap-1.5 rounded-lg bg-brand-600 px-2.5 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-brand-500"><Check className="h-3.5 w-3.5" /> Apply to document</button>
                              )}
                              {m.apply && m.applied && (
              ...
              <div className="flex flex-wrap gap-1.5 border-t border-white/10 px-4 py-2.5" data-testid="assistant-suggestions">
                {suggestions.map((s) => (
                  <button key={s.command} disabled={busy}
                    ...
                    className="inline-flex items-center gap-1 rounded-full border border-brand-500/25 bg-brand-500/[0.06] px-2.5 py-1 text-xs font-medium text-brand-200 transition-all hover:border-brand-500/50 hover:bg-brand-500/15 disabled:opacity-50">
            ...
            <div className="border-t border-white/10 p-3">
              <div className="relative flex items-end gap-2 rounded-xl border border-white/10 bg-zinc-900 p-1.5 focus-within:border-brand-500/50">
