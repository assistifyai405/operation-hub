...
const ITEMS = [
  ...
  { key: "plan", label: "Generate an AI project plan", to: "/projects" },
  { key: "proposal", label: "Generate a proposal", to: "/projects" },
  { key: "contract", label: "Generate a contract", to: "/projects" },
  { key: "invoice", label: "Generate an invoice", to: "/projects" },
];
...
export default function OnboardingChecklist() {
  ...
  useEffect(() => {
    onboardingApi.get().then(setData).catch(() => {});
  }, []);
    ...
    <div className="rounded-xl border border-violet-500/25 bg-gradient-to-br from-violet-600/10 to-transparent p-5" data-testid="onboarding-checklist">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500">Complete these steps to unlock the full workflow.</p>
          </div>
        </div>
      ...
      <div className="mt-4">
        ...
        <div className="h-2 overflow-hidden rounded-full bg-zinc-800">
          <div className="h-full rounded-full bg-violet-500 transition-all" style={{ width: `${data.percent}%` }} />
        </div>
      </div>

      <div className="mt-4 grid grid-cols-1 gap-1.5 sm:grid-cols-2">
        {ITEMS.map((it) => {
          const done = data.checklist[it.key];
          return (
            <button key={it.key} onClick={() => navigate(it.to)} data-testid={`checklist-item-${it.key}`}
              ...
              <span className={`flex-1 text-sm ${done ? "text-zinc-500 line-through" : "text-zinc-200"}`}>{it.label}</span>
              {!done && <ChevronRight className="h-4 w-4 text-zinc-600 opacity-0 transition-opacity group-hover:opacity-100" />}
            </button>
          );
        })}
      </div>
