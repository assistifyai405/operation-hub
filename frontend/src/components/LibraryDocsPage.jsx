...
export default function LibraryDocsPage({ icon: Icon, kind, tabId, statuses, apiFn, help, testid }) {
    ...
    <div className="space-y-5" data-testid={`${testid}-page`}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="flex items-center gap-2 text-sm text-zinc-400">
          {data.total} {kind}{data.total !== 1 && "s"} across your organization.
          <HelpTip testid={`${testid}-help`} text={help} />
        </p>
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-500" />
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={`Search ${kind}s`} data-testid={`${testid}-search`}
      ...
      {loading ? (
        ...
        <EmptyState icon={Icon} title={q || status !== "All" ? `No matching ${kind}s` : `No ${kind}s yet`}
          description={q || status !== "All" ? "Try a different search or filter." : `Open a project and use the ${kind} generator to create your first one.`}
          actionLabel={q || status !== "All" ? undefined : "Go to Projects"} onAction={() => navigate("/projects")} testid={`${testid}-empty`} />
      ) : (
        <>
          <div className="space-y-3">
            {data.items.map((item) => (
              <div key={item.id} className="flex flex-col gap-3 rounded-xl border border-white/10 bg-zinc-950 p-4 transition-all hover:border-violet-500/40 sm:flex-row sm:items-center" data-testid={`${testid}-item-${item.id}`}>
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-violet-600/15 text-violet-400"><Icon className="h-5 w-5" /></div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-zinc-100">{item.title}</p>
                  <p className="truncate text-xs text-zinc-500">{item.client_name || "No client"} · {item.project_name || "No project"} · v{item.version || 1} · {fmtDate(item.updated_at || item.created_at)}</p>
                </div>
                <span className={`inline-flex w-fit items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${statusBadge[item.status] || statusBadge.Draft}`}>{item.status}</span>
                <div className="flex items-center gap-2">
                  <button onClick={() => open(item, false)} data-testid={`${testid}-open-project-${item.id}`} className="rounded-lg border border-white/10 bg-zinc-900 px-3 py-1.5 text-xs text-zinc-300 transition-all hover:border-violet-500/40"><FolderOpen className="mr-1 inline h-3.5 w-3.5" />Project</button>
                  <button onClick={() => open(item, true)} data-testid={`${testid}-open-${item.id}`} className="rounded-lg bg-violet-600 px-3 py-1.5 text-xs font-semibold text-white transition-all hover:bg-violet-500"><ExternalLink className="mr-1 inline h-3.5 w-3.5" />Open</button>
                </div>
              </div>
            ))}
          </div>
