          ...
          {all.map((d, i) => {
            ...
            return (
              <motion.button key={`${d.type}-${d.id}`} onClick={() => navigate(d.link)} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                className="flex flex-col rounded-2xl border border-white/10 bg-zinc-950 p-4 text-left transition-all hover:border-violet-500/40" data-testid={`document-card-${d.type}`}>
                ...
                <div className="mt-auto flex items-center gap-2 pt-2 text-[10px] text-zinc-600">
                  ...
                  {d.confidence ? <span className="text-violet-400">{d.confidence}%</span> : null}
