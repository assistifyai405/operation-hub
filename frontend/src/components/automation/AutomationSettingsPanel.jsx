  ...
  return (
    <div className="space-y-6" data-testid="automation-settings-panel">
      <section className="rounded-2xl border border-white/10 bg-zinc-950 p-5">
        <h3 className="mb-1 text-sm font-semibold text-zinc-100">General</h3>
        <Row title="Enable automations" desc="Master switch. When off, nothing is detected, prepared or run." testid="settings-master-toggle">
          <Switch checked={!!settings.enabled} onCheckedChange={(v) => set({ enabled: v })} data-testid="settings-enabled-switch" />
        </Row>
        <Row title="Default execution mode" desc="Used by any automation that doesn't override it." testid="settings-default-mode">
        ...
        <Row title="Run safe internal actions automatically" desc="Tasks, notes, notifications and internal status changes. Client-facing, AI-generated and destructive actions always need approval." testid="settings-safe-auto">
        ...
        <Row title="Timezone" desc="Used for scheduling and quiet hours." testid="settings-timezone">
      ...
      <section className="rounded-2xl border border-white/10 bg-zinc-950 p-5">
        <h3 className="mb-1 text-sm font-semibold text-zinc-100">Approval preferences</h3>
        <p className="mb-2 text-xs text-zinc-500">These are always on for your safety — external, AI-generated and destructive actions require your explicit confirmation.</p>
        <Row title="Always confirm client-facing actions" desc="Emails and follow-ups sent to clients.">
        ...
        <Row title="Always confirm AI-generated documents" desc="Proposals, contracts and invoices.">
        ...
        <Row title="Always confirm destructive actions" desc="Archiving or deleting anything.">
      ...
      <section className="rounded-2xl border border-white/10 bg-zinc-950 p-5">
        ...
        <Row title="New approvals" desc="Notify me when an action is prepared for approval.">
        ...
        <Row title="Executions" desc="Notify me when an automation runs.">
        ...
        <Row title="Email notifications" desc="Also send these to my email.">
          <Switch checked={!!notif.email} onCheckedChange={(v) => set({ notifications: { ...notif, email: v } })} data-testid="settings-notif-email" />
        </Row>
      </section>

      <section className="rounded-2xl border border-white/10 bg-zinc-950 p-5">
        <h3 className="mb-1 text-sm font-semibold text-zinc-100">Quiet hours</h3>
        <Row title="Pause automatic actions during quiet hours" desc="Detection still runs, but nothing executes automatically and you won't be notified.">
          <Switch checked={!!qh.enabled} onCheckedChange={(v) => set({ quiet_hours: { ...qh, enabled: v } })} data-testid="settings-quiet-toggle" />
        </Row>
        {qh.enabled && (
          <Row title="Quiet window" desc="Between these times, automatic actions are held.">
