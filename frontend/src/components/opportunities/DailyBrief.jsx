...
export function DailyBrief({ data, loading }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-brand-500/20 bg-gradient-to-br from-brand-600/[0.12] to-zinc-950 p-5" data-testid="daily-brief">
      <div className="pointer-events-none absolute -right-8 -top-8 h-32 w-32 rounded-full bg-brand-600/20 blur-3xl" />
      <div className="relative">
        <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-brand-300"><Sparkles className="h-3.5 w-3.5" /> Today's AI Brief</div>
        ...
        <ul className="mt-3 space-y-2">
          {data.lines.map((l, i) => (
            <li key={i} className="flex items-center gap-2.5 text-sm text-zinc-200 animate-fade-up" style={{ animationDelay: `${i * 60}ms` }} data-testid="brief-line">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-brand-600/15 text-brand-300"><AiIcon name={l.icon} className="h-3.5 w-3.5" /></span>
              {l.text}
            </li>
          ))}
        </ul>
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-3">
          ...
          {data.total > 0 && (
            <button onClick={() => navigate("/opportunities")} data-testid="brief-view-all" className="inline-flex items-center gap-1 text-sm font-medium text-brand-300 transition-colors hover:text-brand-200">
