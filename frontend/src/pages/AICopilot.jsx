    ...
    <div className="mx-auto flex h-[calc(100vh-9rem)] max-w-3xl flex-col" data-testid="copilot-page">
      {empty ? (
        <div className="flex flex-1 flex-col items-center justify-center text-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-violet-600 glow-violet"><Sparkles className="h-8 w-8 text-white" /></div>
            ...
            {suggestions.map((s, i) => {
              ...
              return (
                <button key={i} onClick={() => send(s.prompt)} data-testid={`copilot-suggestion-${i}`}
                  className="flex items-center gap-3 rounded-xl border border-white/10 bg-zinc-950 p-3 text-left transition-all hover:border-violet-500/40">
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-violet-600/15 text-violet-400"><Icon className="h-4 w-4" /></span>
      ...
      <form onSubmit={(e) => { e.preventDefault(); send(); }} className="mt-2 flex items-center gap-2 rounded-xl border border-white/10 bg-zinc-950 p-2">
        <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask anything or say 'Create a project for Nike'…" data-testid="copilot-input"
          className="flex-1 bg-transparent px-3 py-2 text-sm text-zinc-100 outline-none placeholder:text-zinc-600" />
        <button type="submit" disabled={loading || !input.trim()} data-testid="copilot-send" className="flex h-9 w-9 items-center justify-center rounded-lg bg-violet-600 text-white transition-all hover:bg-violet-500 disabled:opacity-40">
