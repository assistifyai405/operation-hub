            ...
            {(asArray(suggestions).length
              ...
              return (
                <button key={i} type="button" onClick={() => send(s.prompt)} data-testid={`copilot-suggestion-${i}`}
                  className="flex items-center gap-3 rounded-xl border border-white/10 bg-zinc-950 p-3 text-left transition-all hover:border-brand-500/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-brand-600/15 text-brand-400"><Icon className="h-4 w-4" aria-hidden="true" /></span>
        ...
        <div className="flex-1 space-y-4 overflow-y-auto pb-4" data-testid="copilot-messages">
          {messages.map((m, i) => (
            <div key={i} className={`flex gap-3 ${m.role === "user" ? "justify-end" : ""}`}>
              {m.role === "assistant" && <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-brand-600/15 text-brand-400"><Sparkles className="h-4 w-4" /></div>}
              <div className={`max-w-[80%] ${m.role === "user" ? "order-1" : ""}`}>
                <div className={`rounded-2xl px-4 py-2.5 text-sm ${m.role === "user" ? "bg-brand-600 text-white" : m.error ? "border border-red-500/30 bg-red-500/10 text-red-200" : "border border-white/10 bg-zinc-950 text-zinc-200"}`}
          ...
          {loading && <div className="flex gap-3"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-brand-600/15 text-brand-400"><Sparkles className="h-4 w-4" /></div><div className="rounded-2xl border border-white/10 bg-zinc-950 px-4 py-3"><Loader2 className="h-4 w-4 animate-spin text-brand-400" /></div></div>}
          <div ref={endRef} />
        </div>
      )}
