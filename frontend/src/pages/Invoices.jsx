...
export default function Invoices() {
  ...
  const stats = [
    { label: "Revenue (Paid)", value: money(totals.revenue), icon: TrendingUp, color: "text-emerald-400" },
    { label: "Outstanding", value: money(totals.outstanding), icon: Clock, color: "text-amber-400" },
    { label: "Total invoices", value: totals.count, icon: FileStack, color: "text-brand-400" },
  ];
              ...
              <tbody className="divide-y divide-white/5">
                {data.items.map((inv) => (
                  <tr key={inv.id} className="bg-zinc-950/50 transition-colors hover:bg-zinc-900/50" data-testid={`invoice-row-${inv.id}`}>
                    ...
                    <td className="hidden px-4 py-3 text-zinc-400 md:table-cell">{formatDate(inv.updated_at || inv.created_at, i18n.resolvedLanguage, { month: "short" })}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => navigate(`/projects/${inv.project_id}?tab=invoice`)} data-testid={`invoice-open-${inv.id}`} className="rounded-lg bg-brand-600 px-3 py-1.5 text-xs font-semibold text-white transition-all hover:bg-brand-500"><ExternalLink className="mr-1 inline h-3.5 w-3.5" />Open</button>
                    </td>
                  </tr>
                ))}
              </tbody>
