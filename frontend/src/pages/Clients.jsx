    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="border-white/10 bg-zinc-950 text-zinc-100 sm:max-w-md" data-testid="client-dialog">
        ...
        <DialogFooter>
          ...
          <button onClick={submit} disabled={saving} data-testid="client-save-btn" className="flex items-center gap-2 rounded-lg bg-violet-600 px-4 py-2 text-sm font-semibold text-white transition-all hover:bg-violet-500 disabled:opacity-50">
            ...
            <tbody>
              {filtered.map((c) => (
                <tr key={c.id} className="border-b border-white/5 transition-colors hover:bg-zinc-900/40" data-testid={`client-row-${c.id}`}>
                  ...
                  <td className="hidden px-5 py-3 font-semibold text-violet-400 md:table-cell">{fmt(c.value)}</td>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => openEdit(c)} data-testid={`edit-client-${c.id}`} className="rounded-md p-1.5 text-zinc-500 transition-colors hover:bg-zinc-800 hover:text-violet-400"><Pencil className="h-4 w-4" /></button>
